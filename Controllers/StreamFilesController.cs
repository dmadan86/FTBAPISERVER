...
namespace FTBAPISERVER.Controllers
{
    ...
    public class StreamFilesController : ApiController
    {
        ...
        public FileMetaData GetFileMetaData(string fileName)
        {
            try
            {
                string filePath = fileName;// string.Concat(this.GetDownloadPath(), "\\", fileName);
                ...
                if (!fileInfo.Exists)
                {
                    ...
                    return metaData; // JsonConvert.SerializeObject(metaData); // Request.CreateResponse(HttpStatusCode.NotFound, metaData, new MediaTypeHeaderValue("text/json"));
                }
                metaData.FileResponseMessage.IsExists = true;
                metaData.FileName = fileInfo.Name; // fileName;
                metaData.FileExtension = fileInfo.Extension;
                metaData.FileSize = fileInfo.Length;
                metaData.FilePath = filePath;
                metaData.FileCreation = fileInfo.CreationTimeUtc;  
                metaData.FileResponseMessage.Content = string.Format("{0} file is found !", fileName);
                return metaData; // JsonConvert.SerializeObject(metaData);//  Request.CreateResponse(HttpStatusCode.OK, metaData, new MediaTypeHeaderValue("text/json"));
            }
            catch (Exception exception)
            {
                return new FileMetaData();// exception.Message;// JsonConvert.Serialize();// Request.CreateErrorResponse(HttpStatusCode.InternalServerError, exception);
            }
        ...
        private HttpResponseMessage ProcessUploadRequest(bool overWrite)
        {
            ...
            if (this.GetRequestContentLength() > MvcApplication.MaxRequestLength)
            {
                int maxSize = Convert.ToInt32(Math.Round(MvcApplication.MaxRequestLength / 1024.0, 1));
